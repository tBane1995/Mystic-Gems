...
public class Game extends Layout {
    ...
    private List<Gem> _gems = new ArrayList<>();
    private final List<ComboText> _comboTexts = new ArrayList<>();
    ...
    /// /////////////////////////////////////////
    enum GameStates { Start, Playing, LevelComplete, NextLevelText, UpdateHighscores, EndScreen, End }
    private GameStates _state;
    private float _lastTime;
    /// ////////////////////////////////
    private final Rectangle _topRect;
    private final Rectangle _bottomRect;
    private final Rectangle _gameBoardRect;
    /// ////////////////////////////////
    ...
    private final ButtonWithText _exitBtn;
    ...
    private void createGems() {
        ...
        int gemsCols = Renderer.VIRTUAL_WIDTH/(int)Gem.size.x;
        int gemsRows = (int)Math.ceil((double)gemsCount/gemsCols);
        gemsRows *= 5;

        int dx = (Renderer.VIRTUAL_WIDTH - gemsCols*(int)Gem.size.x)/2;
    ...
    private void destroyAllGems(){
        for (int i = _gems.size() - 1; i >= 0; i--) {
            ...
            if(rect.overlaps(_gameBoardRect)){
                if(_gems.get(i).effect == Gem.GemEffect.BonusPoints){
    ...
    void destroyType(int colorType) {
        for (int i = _gems.size() - 1; i >= 0; i--) {
            ...
            if(rect.overlaps(_gameBoardRect) && _gems.get(i).color == colorType){
                ...
                if(_gems.get(i).effect == Gem.GemEffect.DestroyAll){
                    destroyAllGems();
                }else if(_gems.get(i).effect == Gem.GemEffect.BonusPoints){
                    createFragments(_gems.get(i));
                    _points += 1;
                    _hittedGems += 1;
                    _combo+=1;
    ...
    private void findRankingPosition() {
        ...
        int insertIndex = _rankingNames.size();
    ...
    private void loadHighscores() {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (!prefs.contains("point" + i))
                break; // nie ma więcej zapisanych wyników

            _rankingNames.add(prefs.getString("name" + i));
            _rankingPoints.add(prefs.getInteger("point" + i));
            _rankingCombos.add(prefs.getInteger("combo" + i));
        }
    }

    private void addToHighscores() {
        // Wstaw nowy wynik na odpowiednią pozycję
        ...
        // Ogranicz ranking do maksymalnie 15 wpisów
        while (_rankingPoints.size() > 15) {
            int last = _rankingPoints.size() - 1;
            _rankingNames.remove(last);
            _rankingPoints.remove(last);
            _rankingCombos.remove(last);
        }
    }
    ...
    private void drawEndGameScreen() {
        ...
        Texture texture = AssetsManager.getTexture(("tex/smallPanel.png"));
        if(texture != null){
    ...
    public void update() {
        ...
        switch(_state){
            ...
            case End:
                ...
                if(_restartBtn._state == Button.states.Idle && _exitBtn._state == Button.states.Idle){
                    SoundManager.stopAll();
                    SoundManager.playMenuMusic();
                    LayoutsManager.pop_back();
                }
    ...
    public void draw() {
        ...
        Texture bgTexture = AssetsManager.getTexture("tex/mainBoard.png");
        if(bgTexture != null){
        ...
        Texture gameBoardTexture = AssetsManager.getTexture("tex/underwater.png");
        if(gameBoardTexture!=null){
            Sprite gameBoard = new Sprite(gameBoardTexture);
        ...
        Texture topBoardTexture = AssetsManager.getTexture("tex/topBoard.png");
        if(topBoardTexture!=null){
            Sprite topBoard = new Sprite(topBoardTexture);
            topBoard.setPosition(_topRect.x, _topRect.y);
            topBoard.draw(Renderer.batch);
        }

        Texture bottomBoardTexture = AssetsManager.getTexture("tex/bottomBoard.png");
        if(bottomBoardTexture!=null){
            Sprite bottomBoard = new Sprite(bottomBoardTexture);
        ...
        Font.gameTopTextFont.draw(Renderer.batch, "combo", (float)Renderer.VIRTUAL_WIDTH / 2 - textWidth / 2, Renderer.VIRTUAL_HEIGHT - 16);
        ...
        textWidth = layout.width;
        Font.gameTopTextFont.draw(Renderer.batch, Integer.toString(_combo), (float)Renderer.VIRTUAL_WIDTH / 2 - textWidth / 2, Renderer.VIRTUAL_HEIGHT - 16 - y);
        ...
        // draw the big Gem
        Texture gemTex = AssetsManager.getTexture(_targetColor);
        if (gemTex != null) {
            Sprite gemSprite = new Sprite(gemTex);
