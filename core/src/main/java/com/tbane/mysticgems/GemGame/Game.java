...
public class Game extends Layout {
    ...
    private void generateEndScreenComment(){
        ...
        for(int i=0;i<10;i+=1){
            ...
            points.add(prefs.getInteger("point" + i, -1));
            combos.add(prefs.getInteger("combo" + i, -1));
        }

        endScreenComment = "GAME OVER";

        if (_points > points.get(0)) {
            endScreenComment = "NEW RECORD";
        }
        else if (_points > points.get(1)) {
            endScreenComment = "SECOND PLACE";
        }
        else if (_points > points.get(4)) {
            endScreenComment = "TOP 5";
        }
        else if (_points > points.get(9)) {
            endScreenComment = "TOP 10";
        }else if (_points > points.get(14)) {
    ...
    private void saveHighscores() {
        ...
        names.add("nemo");
        points.add(_points);
        combos.add(_maxCombo);

        // sort highscores
        for (int i = 0; i < points.size(); i++) {
            for (int j = i + 1; j < points.size(); j++) {
                if (points.get(j) > points.get(i)) {
                    int tmpPoints = points.get(i);
                    points.set(i, points.get(j));
                    points.set(j, tmpPoints);
                    ...
                    // zamiana combo
                    int tmpCombo = combos.get(i);
                    combos.set(i, combos.get(j));
                    combos.set(j, tmpCombo);
                }
            }
        }
        ...
        // save highscores
        for (int i = 0; i < 10; i++) {
            prefs.putString("name" + i, names.get(i));
            prefs.putInteger("point" + i, points.get(i));
            prefs.putInteger("combo" + i, combos.get(i));
        }
    ...
    private void drawEndGameScreen() {
        ...
        Font.rankingTitleFont.draw(Renderer.batch, endScreenComment, (x - textWdt/2.0f), (y + textHgh + 160));
        ...
        Font.GameCommentFont.draw(Renderer.batch, "points:", (x - 224 + 16), (y + textHgh));

        layout = new GlyphLayout();
        layout.setText(Font.GameCommentFont, Integer.toString(_points));
        textWdt = layout.width;
        textHgh = Font.GameCommentFont.getCapHeight();
        Font.GameCommentFont.draw(Renderer.batch, Integer.toString(_points), (x - textWdt + 192), (y + textHgh));
        ...
        textWdt = layout.width;
        textHgh = Font.GameCommentFont.getCapHeight();
        Font.GameCommentFont.draw(Renderer.batch, "max combo:", (x - 224 + 16), (y + textHgh - 64));

        layout = new GlyphLayout();
        layout.setText(Font.GameCommentFont, Integer.toString(_maxCombo));
        textWdt = layout.width;
        textHgh = Font.GameCommentFont.getCapHeight();
        Font.GameCommentFont.draw(Renderer.batch, Integer.toString(_maxCombo), (x - textWdt + 192), (y + textHgh - 64));
        ...
        textWdt = layout.width;
        textHgh = Font.GameCommentFont.getCapHeight();
        Font.GameCommentFont.draw(Renderer.batch, "level:", (x - 224 + 16), (y + textHgh - 128));

        layout = new GlyphLayout();
        layout.setText(Font.GameCommentFont, Integer.toString(_level));
        textWdt = layout.width;
        textHgh = Font.GameCommentFont.getCapHeight();
        Font.GameCommentFont.draw(Renderer.batch, Integer.toString(_level), (x - textWdt + 192), (y + textHgh - 128));
    ...
    public void handleEvents() {
        if (MyInput.processor.isTouchDown()) {
            float x = MyInput.processor.getTouchX();
            float y = MyInput.processor.getTouchY();
            System.out.println("xy: " + Float.toString(x) + ", " + Float.toString(y) + "\n");
            if (_gameBoardRect.contains(x, y)) {
                Cursor.setOn(x,y);
                if(_state == GameStates.Playing)
                    destroyClickedGems();
            }
        }
        else if (MyInput.processor.isBackPressed()) {
            // back btn
